...
class AddFriendsBucket extends StatefulWidget {
  final name;
  AddFriendsBucket(this.name);


  @override
  State<AddFriendsBucket> createState() => _AddFriendsBucket(name);
}

class _AddFriendsBucket extends State<AddFriendsBucket> {
  final name;
  _AddFriendsBucket(this.name);
                    ...
                    return ListTile(
                      title: TextButton(
                          onPressed: () {
                            ...
                            FirebaseFirestore.instance.collection(data["email"]!).doc("buckets").collection("bucketNames").doc(name).set({"name": name});
                            FirebaseFirestore.instance.collection(data["email"]!).doc("buckets").collection("bucketValue").doc(name).set({"value": false});

                            FirebaseFirestore.instance.collection(userEmail!).doc("buckets").collection("DreamsInBucket").doc(name).collection("name").doc(name).set({"name": name});
                            FirebaseFirestore.instance.collection(userEmail!).doc("buckets").collection("DreamsInBucket").doc(name).collection("value").doc(name).set({"value": true});
